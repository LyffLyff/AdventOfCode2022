...
# adding every fixed/scanned position to array -> [[x,y],[x,y],....]
for i in input:
...
while True:
  if current_sandcorn_pos[1] + 1 == FLOOR_LEVEL:
    # sandcorn in fixed position
    fixed_rocks_positions.append(current_sandcorn_pos)
    current_sandcorn_pos = start_pos
    landed_sandcorns += 1
  elif [current_sandcorn_pos[0], current_sandcorn_pos[1] + 1] not in fixed_rocks_positions:
    # sandcorn moves down
    ...
    # sandcorn in fixed position
    landed_sandcorns += 1
    if current_sandcorn_pos == start_pos:
      break
...
print("How many Sandcorns it took for the stack to reach the start pos: ", landed_sandcorns)